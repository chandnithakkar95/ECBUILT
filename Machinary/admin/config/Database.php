...
var $database = "machinary"; //database name
...
#-#############################################
# desc: constructor
function Database($dbsource = 'default')
{
	$this->connect();
...
function query_delete($table,$condition,$fields = ""){
	if(empty($fields))
	...
	if($r)
	{
		$rows = mysql_affected_rows($this->link_id);
		if ($rows == 0)
...
function oops($msg = '') {
	...
	?>
		...
		<tr><td align="right">Script:</td><td><a href="<?php echo @$_SERVER['../../../../nease/classes/libraries/REQUEST_URI']; ?>"><?php echo @$_SERVER['REQUEST_URI']; ?></a></td></tr>
